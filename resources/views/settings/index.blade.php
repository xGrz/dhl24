        ...
        <div class="p-2">
            <x-p::table>
                <x-p::table.head>
                    ...
                    <x-p::table.th>Options</x-p::table.th>
                </x-p::table.head>
                <x-p::table.body>
                    @foreach($costCenters as $center)
                        <x-p::table.row>
                            <x-p::table.cell>{{ $center->name }}</x-p::table.cell>
                            <x-p::table.cell>{{ $center->is_default }}</x-p::table.cell>
                            <x-p::table.cell>
                                <x-p::buttonlink
                                    href="{{route('dhl24.costCenters.destroy', $center->id)}}"
                                    color="danger"
                                    size="small"
                                >
                                    Delete
